...
def add_reentrant_protection(
    ...
    """Protect text/program syntax while allowing explicitly delimited math.
    ...
    if span is None:
        return
    cursor, limit = span
    for start, end in explicit_math_spans(tokens):
        if cursor < start:
...
def classify_unit(
    ...
    # dimensions remain literal; only explicit math labels reopen notation
    # classification.  Installed tikz.code.tex puts node text in an hbox
    ...
    for formula_index, (formula_kind, tokens) in enumerate(analysis.formulas):
        for token in tokens:
            if token.kind == "command":
        ...
        protections = collect_protections(
            tokens, policy, protected_unit=unit_id in protected_ids,
        )
        protections.extend(program_protections)
        math_field_boundaries = collect_math_field_boundaries(tokens, protections)
        formula_keys = collect_formula_keys(tokens)
        pos = 0
        while pos < len(tokens):
            token = tokens[pos]
            if token.kind == "command" and token.value[1:] in GREEK_COMMANDS:
    ...
    formula_spans = [
        ...
        for _, tokens in analysis.formulas if tokens
    ]
    ...
    summary = {
        "formula_segments": len(analysis.formulas),
...
def build_payload(
    ...
    for item in units:
        ...
        occurrences, replacements, unit_summary = classify_unit(
            unit_id, text, bom, policy, greek_registry,
        )
