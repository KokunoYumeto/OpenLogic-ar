...
def validate_fls(directory: Path, repo: Path, manifest: dict, job: str,
    ...
    # MiKTeX records its deleted engine self-test scratch file even when the
    # process TEMP/TMP variables point at the isolated build directory. Admit
    # only that one exact, already-removed user-data path; every durable TeX
    # product must still remain below the build directory.
    local_app_data = os.environ.get("LOCALAPPDATA")
    require(bool(local_app_data), "LOCALAPPDATA is unavailable")
    expected_miktex_scratch = (
        Path(local_app_data) / "MiKTeX" / "m_t_x_t_e_s_t.tmp"
    ).resolve()
    external_outputs = {item for item in outputs if not item.is_relative_to(directory)}
    require(external_outputs == {expected_miktex_scratch} and
            not expected_miktex_scratch.exists(),
    ...
    return {
        ...
        "external_ephemeral_outputs": ["@MIKTEX_USERDATA@/m_t_x_t_e_s_t.tmp"],
    }
