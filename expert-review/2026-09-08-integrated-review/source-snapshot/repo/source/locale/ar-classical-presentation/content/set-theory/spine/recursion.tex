...
\begin{cases}
\OLId{latin-looped}{A} &\text{إذا كان $x = \emptyset$، أو لم يكن $x$ دالة}\\
&\hspace{1em}\text{مجالها عدد ترتيبي؛ وخلاف ذلك:}\\
\tau(\OLId{latin-ordinary}{x}(\OLId{greek-variable}{alpha})) & \text{إذا كان $\dom{x} = \ordsucc{\alpha}$}\\
\theta(\ran{\OLId{latin-ordinary}{x}}) & \text{إذا كان $\dom{x}$ عددًا ترتيبيًا حديًا}
\end{cases}
\]
