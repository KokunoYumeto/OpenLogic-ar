...
try:
    from build import verify_three_exact_locators_20260909 as exact_locator_readback
    from build import verify_olp0014_note_review_20260912 as note_readback
except ModuleNotFoundError:
    import verify_three_exact_locators_20260909 as exact_locator_readback
    import verify_olp0014_note_review_20260912 as note_readback
...
def mp_nfc_predecessor(repo, uid, ledger_identity, declaration, current, inputs):
    """Independently replay only the pinned three mark-order transitions."""
    receipt = "evidence/classical/repairs/msa-mp-nfc-20260909/NORMALIZATION_TRANSITIONS.json"
    receipt_hash = "bab1bc85dd51ce07232f0012c822620b9bdd53186b33f0accd45a5c4428beb65"
    raw = bounded_bytes(repo / receipt)
    require(len(raw) == 9169 and byte_digest(raw) == receipt_hash, "NFC transition receipt changed")
    payload = json.loads(raw)
    require(payload["schema"] == "openlogic-msa-nfc-normalization-transition-v1" and len(payload["transitions"]) == 3
            and {r["unit_id"] for r in payload["transitions"]} == {"OLP-0079", "OLP-0093", "OLP-0497"}, "NFC transition inventory changed")
    matches = [r for r in payload["transitions"] if r["unit_id"] == uid]
    require(len(matches) == 1, "uncommissioned NFC unit")
    item = matches[0]
    require(item["target_path"] == declaration["path"] and item["historical_ledger"]["path"] == ledger_identity["path"], "NFC source/ledger differs")
    ledger = next(r for r in payload["historical_ledgers"] if r["path"] == ledger_identity["path"])
    ledger_raw = bounded_bytes(repo / ledger["path"])
    require(byte_digest(ledger_raw) == ledger["sha256"] == ledger_identity["sha256"]
            and len(ledger_raw) == ledger["bytes"] == ledger_identity["bytes"], "NFC historical ledger changed")
    selected = json.loads(ledger_raw)
    for part in item["historical_ledger"]["msa_pointer"].strip("/").split("/"):
        selected = selected[int(part)] if isinstance(selected, list) else selected[part]
    require(selected == declaration and item["predecessor"] == {
        "sha256": declaration["after_sha256"].lower(), "bytes": declaration["after_bytes"]}, "NFC historical pointer differs")
    require(byte_digest(current) == item["current"]["sha256"] and len(current) == item["current"]["bytes"], "NFC live bytes differ")
    patch = item["patch"]
    old, new = bytes.fromhex(patch["old_utf8_hex"]), bytes.fromhex(patch["new_utf8_hex"])
    start, end = patch["byte_start"], patch["byte_end_exclusive"]
    require(old == patch["old_text"].encode() and new == patch["new_text"].encode() and old != new
            and len(old) == len(new) == end - start == 16
            and current[start:end] == new and current.count(new) == patch["occurrences"] == 1
            and current[:start].count(b"\n") + 1 == patch["line"], "NFC byte-offset literal differs")
    for phase in ("old", "new"):
        require([f"U+{ord(char):04X}" for char in patch[phase + "_text"]] == patch[phase + "_codepoints"], "NFC codepoints differ")
    prior = current[:start] + old + current[end:]
    require(byte_digest(prior) == item["predecessor"]["sha256"] and len(prior) == item["predecessor"]["bytes"]
            and prior[:start] + new + prior[end:] == current, "NFC exact inverse/forward replay failed")
    require(unicodedata.normalize("NFC", prior.decode()) == current.decode(), "NFC equivalence failed")
    inputs[str(repo / receipt)] = receipt_hash
    return prior, {"receipt_path": receipt, "receipt_sha256": receipt_hash, "transition": item}
...
def repair_expectations(repo, english_root):
    ...
    for name, commissioned in REPAIR_LEDGERS.items():
        ...
        for row in rows:
            ...
            witness_refreshes = []
            cardinality_current = {}
            nfc_current, encoding_transition = None, None
            base = units[unit_id]
            for kind, field, term_field in (("english", "source_path", "source_term"),
                ...
                if unit_id == "OLP-0033" and kind in {"msa", "classical"}:
                    ...
                    data = bounded_bytes(target)
                    inventory[str(target)] = byte_digest(data)
                if kind == "msa" and not qualification and (
                        (repo / "evidence/classical/repairs/msa-mp-nfc-20260909/NORMALIZATION_TRANSITIONS.json").exists()
                        or not same_hash(byte_digest(data), declaration["after_sha256"])):
                    nfc_current = data
                    data, encoding_transition = mp_nfc_predecessor(repo, unit_id, identity, declaration, data, inventory)
                if kind == "english":
            ...
            if nfc_current is not None:
                historical_msa = witnesses["msa"]
                fresh = {"path": historical_msa["path"], "sha256": byte_digest(nfc_current),
                         "bytes": len(nfc_current), "line_start": historical_msa["line_start"],
                         "line_end": historical_msa["line_end"]}
                witnesses["msa"] = declared_witness(nfc_current, fresh, historical_msa["term"], "msa")
                spec["encoding_transition"] = encoding_transition
...
def validate_repair_record(row, spec, errors):
    ...
    if repair.get("encoding_transition") != spec.get("encoding_transition"):
        problem("exact historical/current encoding transition differs")
...
def verify_canon_amendment_v2(repo, entry, english_root):
    """Independent v2 readback: literal contexts and byte identity, not OCR truth."""
    inputs, excerpts = {}, []
    def check(condition, message):
        require(condition, entry["decision_id"] + ": canon readback " + message)
    def nonempty(value):
        return isinstance(value, str) and bool(value.strip())
    check(not {"amendment_schema", "original_fields", "ledger_path"}.intersection(entry),
          "reserved generated provenance supplied in ledger")
    check(entry.get("recording_mode") == "new-independent-semantic-assessment"
          and entry.get("open_to_correction") is True, "assessment provenance differs")
    changes = entry.get("changes", {})
    required = {"sense", "rationale", "expert_question"}
    check(set(changes) in (required, required | {"expert_review_useful", "expert_review_reason"})
          and all(nonempty(changes[k]) for k in required), "assessment changes differ")
    if "expert_review_useful" in changes:
        check(changes["expert_review_useful"] is True and nonempty(changes["expert_review_reason"]),
              "sensitive review flag may only be promoted with a reason")
    witnesses = entry.get("witnesses", [])
    check(isinstance(witnesses, list) and bool(witnesses), "missing translation witnesses")
    for witness in witnesses:
        kind, relative = witness.get("source_kind"), witness.get("path")
        prefixes = {"english": "content/", "msa": "source/locale/ar/", "classical": "source/locale/ar-classical/"}
        check(kind in prefixes and isinstance(relative, str) and "\\" not in relative
              and relative.startswith(prefixes[kind]), "witness language/path invalid")
        base = english_root if kind == "english" else repo
        target = (base / relative).resolve()
        check(target.is_relative_to((base / prefixes[kind]).resolve()), "witness path escapes source")
        data = bounded_bytes(target)
        check(same_hash(byte_digest(data), witness.get("file_sha256")), "witness hash changed")
        lines = data.decode("utf-8-sig").splitlines()
        start, end = witness.get("line_start"), witness.get("line_end")
        check(type(start) is int and type(end) is int and 1 <= start <= end <= len(lines), "witness range invalid")
        excerpt = "\n".join(lines[start - 1:end])
        check(same_hash(byte_digest(excerpt.encode("utf-8")), witness.get("excerpt_sha256"))
              and nonempty(witness.get("semantic_basis")), "witness excerpt/basis differs")
        excerpts.append(excerpt)
        inputs[str(target)] = byte_digest(data)
    check(any(w["source_kind"] == "english" for w in witnesses)
          and any(w["source_kind"] in ("msa", "classical") for w in witnesses), "both languages required")
    def canon_file(item, suffix):
        check(isinstance(item, dict), "file identity missing")
        relative = item.get("path")
        check(isinstance(relative, str) and "\\" not in relative and not Path(relative).is_absolute(),
              "file path invalid")
        target = (repo / relative).resolve()
        roots = [(repo / "evidence").resolve(), (repo.parent / "sources").resolve()]
        if suffix == ".png":
            roots.append((repo / "tmp/pdfs").resolve())
        check(any(target.is_relative_to(root) for root in roots) and target.suffix.lower() == suffix,
              "file outside canon roots")
        check(type(item.get("bytes")) is int and item["bytes"] > 0
              and target.stat().st_size == item["bytes"], "file bytes changed")
        actual = inputs.get(str(target)) or digest(target)
        check(same_hash(actual, item.get("sha256")), "file hash changed")
        inputs[str(target)] = actual
    evidence = entry.get("canon_evidence")
    check(isinstance(evidence, list) and 0 < len(evidence) <= 20, "bounded evidence inventory absent")
    forms = {item["form"] for item in entry.get("observed_arabic_forms", [])} or {entry["expected"]["chosen_arabic"]}
    exact = set()
    for item in evidence:
        check(isinstance(item, dict), "evidence item malformed")
        for field in ("printed_page", "entry", "exact_passage", "chosen_arabic_form", "attested_arabic_form",
                      "english_headword", "english_source_phrase", "english_sense", "relevance", "limits"):
            check(nonempty(item.get(field)), "missing " + field)
        check(type(item.get("physical_page")) is int and item["physical_page"] > 0, "physical page invalid")
        check(len(item["exact_passage"]) <= 1200 and same_hash(
            byte_digest(item["exact_passage"].encode("utf-8")), item.get("passage_sha256")), "passage changed")
        check(item.get("transcription_mode") == "manual-visual-transcription-diacritics-normalized"
              and item.get("consulted_before_assessment") is True
              and item.get("recording_mode") == "retrospective-canon-check", "consultation provenance differs")
        check(date.fromisoformat(item["consulted_on"]) <= date.fromisoformat(entry["assessed_on"]),
              "consultation postdates assessment")
        source = item.get("source")
        canon_file(source, ".pdf")
        canon_file(item.get("page_image"), ".png")
        check(all(nonempty(source.get(k)) for k in ("title", "source_id", "url"))
              and re.fullmatch(r"https?://[^\s<>]+", source["url"]), "source metadata incomplete")
        check(item["relation"] in ("exact-chosen-form", "related-not-exact")
              and item["english_sense"] == changes["sense"]
              and item["chosen_arabic_form"] in forms, "relation/chosen form/sense differs")
        check(item["attested_arabic_form"] in item["exact_passage"]
              and item["english_headword"].casefold() in item["exact_passage"].casefold(),
              "dictionary form missing from passage")
        for key, english in (("source_witness_indices", False), ("english_witness_indices", True)):
            indices = item.get(key)
            check(isinstance(indices, list) and bool(indices) and len(indices) == len(set(indices))
                  and all(type(i) is int and 0 <= i < len(witnesses) for i in indices), "context indices invalid")
            for index in indices:
                check((witnesses[index]["source_kind"] == "english") == english, "context language differs")
                phrase = item["english_source_phrase"] if english else item["chosen_arabic_form"]
                # Exact spelling remains material, including Arabic definiteness/inflection.
                active = []
                for line in excerpts[index].splitlines():
                    active.append(re.split(r"(?<!\\)%", line, maxsplit=1)[0])
                pattern = r"(?<![\w-])" + re.escape(phrase) + r"(?![\w-])"
                check(bool(re.search(pattern, "\n".join(active), re.I if english else 0)),
                      "current literal absent from active context")
        if item["relation"] == "exact-chosen-form":
            check(item["attested_arabic_form"] == item["chosen_arabic_form"], "different form falsely called exact")
            exact.add(item["chosen_arabic_form"])
    check(type(entry.get("official_attestation_claimed")) is bool, "claim is not boolean")
    check(not entry["official_attestation_claimed"] or forms <= exact, "unsupported official attestation")
    return inputs


def verify_canon_surfaces(key, approved, assessment, row, surfaces, csv_rows, errors):
    """Require current canon evidence on each relevant full/sharded human surface."""
    def canon_text(value):
        # Independent finite display contract: ordered set difference A\B is
        # written with U+2216 on reader surfaces. Raw evidence remains exact.
        return re.sub(r"(?<![\w\\/:])A\\B(?![\w\\/])", "A∖B", visible(value))
    if assessment.get("amendment_schema") != "openlogic-ar-expert-review-amendments-v2":
        errors.append(key + ": missing v2 assessment identity")
    if assessment.get("canon_evidence") != approved["canon_evidence"]:
        errors.append(key + ": changed/dropped canon evidence")
    editorial = approved.get("current_editorial_assessment") or {}
    comparison = editorial.get("alternative_comparison")
    if editorial and assessment.get("current_editorial_assessment") != editorial:
        errors.append(key + ": changed/dropped current editorial assessment")
    if row.get("official_attestation_claimed") is not approved["official_attestation_claimed"]:
        errors.append(key + ": current official-attestation flag differs")
    originals = assessment.get('original_fields', {})
    for field, expected in approved.get('historical_fields', {}).items():
        if (originals[field] if field in originals else row.get(field)) != expected:
            errors.append(key + ': historical canon-assessment field changed: ' + field)
    tokens = []
    for item in approved["canon_evidence"]:
        tokens.extend((item["chosen_arabic_form"], item["source"]["title"], item["source"]["source_id"],
                       item["source"]["url"], item["exact_passage"],
                       item["relevance"], item["limits"], "Retrospective canon check",
                       f"physical PDF page {item['physical_page']}", f"printed page {item['printed_page']}"))
        tokens.append("Exact chosen-form attestation" if item["relation"] == "exact-chosen-form"
                      else "Related canon evidence; current form is not literally attested")
    for surface, sections in surfaces.items():
        for section in sections.get(key, []):
            if "Fresh canon evidence" not in section or any(canon_text(t) not in visible(section) for t in tokens):
                errors.append(key + ": missing/truncated fresh canon evidence: " + surface)
            if comparison and (canon_text(comparison) not in visible(section)
                               or "Current alternative comparison" not in section):
                errors.append(key + ": missing/truncated current alternative comparison: " + surface)
    for values in csv_rows.get(key, []):
        if len(values) != 13 or any(canon_text(t) not in visible(values[5]) for t in tokens):
            errors.append(key + ": missing/truncated fresh canon evidence in CSV rationale")
        if comparison and (len(values) != 13 or canon_text(comparison) not in visible(values[4])
                           or "Current alternative comparison" not in values[4]):
            errors.append(key + ": missing/truncated current alternative comparison in CSV")


def snapshot_top_level(path, key):
    """Read one small top-level metadata object without loading the decisions."""
    prefix = '  "' + key + '": '
    with path.open(encoding="utf-8-sig") as stream:
        for line in stream:
            if not line.startswith(prefix):
                continue
            first = line[len(prefix):].rstrip().rstrip(",")
            if first not in ("{", "["):
            ...
            raise ValueError("truncated top-level metadata: " + key)
    return None


def verify_frozen_amendment_inventory(repo, inventory):
    """Independently enumerate/hash current inputs; do not import the producer."""
    ...
    actual = {"schema": "openlogic-review-amendment-input-inventory-v1",
              "semantic_amendments": [], "supporting_inputs": []}
    for group, paths in (("semantic_amendments", sorted((repo / directory).glob("*.json"))),
                         ("supporting_inputs", [repo / p for p in fixed if (repo / p).exists()])):
        for path in paths:
            actual[group].append({"path": path.relative_to(repo).as_posix(),
                                  "bytes": path.stat().st_size, "sha256": digest(path)})
    require(actual == inventory, "frozen amendment inventory changed before/during independent readback")
    return [repo / row["path"] for row in inventory["semantic_amendments"]]


def check(repo, snapshot, previous, english_root=None, *, include_new_repairs=True, frozen_inventory=None):
    errors = []
    counts = Counter()
    statuses = Counter()
    expected = {}
    ledger_identities = []
    amendment_schemas, canon_inputs = {}, {}
    if english_root is None:
        english_root = repo.parent.parent / "openlogic-interfarsi/repo/source/upstream"
    generated_inventory = snapshot_top_level(snapshot / "EXPERT_REVIEW_INDEX.json", "frozen_amendment_inventory")
    if frozen_inventory is None:
        frozen_inventory = generated_inventory
    amendment_paths = sorted((repo / "evidence/provenance/locale-ar/expert-review-amendments").glob("*.json"))
    if frozen_inventory is not None:
        try:
            require(generated_inventory == frozen_inventory, "generated inventory differs from transaction inventory")
            amendment_paths = verify_frozen_amendment_inventory(repo, frozen_inventory)
            generated_assessments = snapshot_top_level(snapshot / "EXPERT_REVIEW_INDEX.json", "expert_review_assessments")
            require([{k: (v.lower() if k == "sha256" else v) for k, v in identity.items()
                      if k in {"path", "sha256", "bytes"}} for identity in generated_assessments]
                    == frozen_inventory["semantic_amendments"], "generated assessment identities differ from frozen inventory")
        except (OSError, ValueError, KeyError, TypeError) as exc:
            errors.append("independent frozen inventory verification failed: " + str(exc))
            # Fail closed: do not read paths from an invalid frozen inventory,
            # or silently substitute the newer live directory inventory.
            amendment_paths = []
    locator_entries, locator_history, locator_inputs = {}, {}, {}
    locator_identity = snapshot_top_level(snapshot / "EXPERT_REVIEW_INDEX.json", "exact_locator_amendments")
    if locator_identity is not None or frozen_inventory is not None:
        try:
            locator_entries, locator_history, locator_inputs = exact_locator_readback.expectations(repo, english_root, decisions)
            require(isinstance(locator_identity, dict)
                    and locator_identity.get("amended_location_ids") == sorted(locator_entries)
                    and locator_identity.get("proposal_sha256") == exact_locator_readback.PROPOSAL_SHA
                    and locator_identity.get("raw_occurrences_replaced") == 0,
                    "exact locator admission identity missing/changed")
        except (OSError, ValueError, KeyError, TypeError) as exc:
            errors.append("independent exact locator inputs failed: " + str(exc))
    repairs, repair_ledgers, repair_inputs = {}, [], {}
    note_specs, note_inputs = {}, {}
    try:
    ...
    if include_new_repairs:
        try:
            note_specs, note_inputs = note_readback.expectations(repo, english_root)
        except (OSError, ValueError, KeyError, TypeError) as exc:
            errors.append("independent OLP-0014 clarification input verification failed: " + str(exc))
        try:
    ...
    for path in amendment_paths:
        ...
        for row in payload["amendments"]:
            amendment_schemas[row["decision_id"]] = payload.get("schema")
            if payload.get("schema") == "openlogic-ar-expert-review-amendments-v2":
                try:
                    canon_inputs.update(verify_canon_amendment_v2(repo, row, english_root))
                except (OSError, ValueError, KeyError, TypeError) as exc:
                    errors.append("independent v2 canon admission failed: " + str(exc))
    ...
    for path in shards:
        if path.name.startswith(("complete-", "priority-")):
            ...
            surfaces[path.stem] = decision_sections(body)
    csv_rows = {}
    required_csv_ids = set(expected) | set(repairs) | set(note_specs) | {e["decision_id"] for e in locator_entries.values()}
    try:
    ...
    applied_notes, verified_notes = set(), set()
    ...
    for row in decisions(snapshot / "EXPERT_REVIEW_INDEX.json"):
        ...
        if key in note_specs:
            applied_notes.add(key)
        try:
            if note_readback.validate_row(row, note_specs, surfaces, csv_rows, errors):
                verified_notes.add(key)
        except (ValueError, KeyError, TypeError, AttributeError) as exc:
            errors.append(key + ": invalid OLP-0014 clarification readback: " + str(exc))
        if locator_entries:
            try:
                amended, siblings = exact_locator_readback.validate_row(
                    row, locator_entries, locator_history, surfaces, csv_rows)
                counts["independently_verified_exact_locator_amendments"] += amended
                counts["exact_locator_siblings_preserved"] += siblings
            except (ValueError, KeyError, TypeError) as exc:
        ...
        if not isinstance(assessment, dict):
            ...
            continue
        applied.add(key)
        if amendment_schemas[key] == "openlogic-ar-expert-review-amendments-v2":
            verify_canon_surfaces(key, approved, assessment, row, surfaces, csv_rows, errors)
    ...
    if locator_entries and (counts["independently_verified_exact_locator_amendments"] != 3
                            or counts["exact_locator_siblings_preserved"] != 29):
        errors.append("not all three exact locators and 29 sibling locations independently verified")
    ...
    missing_notes = sorted(set(note_specs) - applied_notes)
    if missing_notes:
        errors.append("commissioned OLP-0014 clarification decisions missing: " + repr(missing_notes))
    counts["source_clarification_assessments_expected"] = len(note_specs)
    counts["applied_source_clarification_assessments"] = len(applied_notes)
    counts["independently_verified_source_clarification_assessments"] = len(verified_notes)
    counts["source_clarification_locations_expected"] = len(note_specs)
    counts["source_clarification_input_files_verified"] = len(note_inputs)
    ...
    for name, expected_hash in canon_inputs.items():
        if not same_hash(digest(Path(name)), expected_hash):
            errors.append("canon input changed during independent readback: " + name)
    for name, expected_hash in locator_inputs.items():
        if not same_hash(digest(Path(name)), expected_hash):
            errors.append("exact locator input changed during independent readback: " + name)
    for name, expected_hash in note_inputs.items():
        if not same_hash(digest(Path(name)), expected_hash):
            errors.append("OLP-0014 clarification input changed during independent readback: " + name)
    if frozen_inventory is not None:
        try:
            verify_frozen_amendment_inventory(repo, frozen_inventory)
        except (OSError, ValueError, KeyError, TypeError) as exc:
            errors.append(str(exc))
    return {
        ...
        "canon_input_sha256": canon_inputs,
        "exact_locator_input_sha256": locator_inputs,
        "source_clarification_ledgers": [next(iter(note_specs.values()))["identity"]] if note_specs else [],
        "source_clarification_input_sha256": note_inputs,
