...
from classical_implicit_carrier_successor_20260910 import (
    CURRENT_SHA256 as IMPLICIT_CARRIER_CONFIG_SHA,
    HISTORICAL_SHA256 as IMPLICIT_CARRIER_CONFIG_HISTORICAL_SHA,
    validate_successor as validate_implicit_carrier_config_successor,
)
from classical_closure_successor_adapter_20260913 import (
    SEMANTIC_ADDITIONS as CLOSURE_SUCCESSOR_SEMANTIC_ADDITIONS,
...
def validate_source_closure(
    ...
    has_successor = "five_source_successor_admission" in closure
    closure_fields = CLOSURE_FIELDS | (
        {"five_source_successor_admission"} if has_successor else set()
    )
    ...
    for baseline_unit, item in zip(baseline_units, units, strict=True):
        ...
        semantic_fields = {"authority_ref", "note"}
        if has_successor and uid == "OLP-0014":
            semantic_fields |= CLOSURE_SUCCESSOR_SEMANTIC_ADDITIONS.keys()
        require(isinstance(semantic, dict) and
                set(semantic) == semantic_fields and
    ...
    if has_successor:
        try:
            validate_successor_closure(repo, closure, baseline, acceptance_artifact)
        except (ValueError, KeyError, TypeError, OSError) as exc:
            raise MaterializationError(f"source-closure successor admission failed: {exc}") from exc
...
def validate_implicit_carrier_authority(repo: Path) -> dict[str, Any]:
    """Bind unchanged carriers to the exact admitted configuration successor."""
    path = repo / IMPLICIT_CARRIER_CONFIG
    raw = path.read_bytes()
    try:
        historical_raw, successor = validate_implicit_carrier_config_successor(raw)
    except ValueError as exc:
        raise MaterializationError(str(exc)) from exc
    lines = raw.decode("utf-8-sig").splitlines()
    historical_lines = historical_raw.decode("utf-8-sig").splitlines()
    for name, (_, number, definition) in IMPLICIT_CARRIERS.items():
        require(len(lines) >= number and len(historical_lines) >= number and
                lines[number - 1] == historical_lines[number - 1] == definition,
                f"implicit math-carrier definition changed: {name}")
    verifier_relative = Path("build/classical_implicit_carrier_successor_20260910.py")
    verifier_raw = (repo / verifier_relative).read_bytes()
    return {"path": IMPLICIT_CARRIER_CONFIG.as_posix(), "bytes": len(raw),
            "sha256": IMPLICIT_CARRIER_CONFIG_SHA,
            "configuration_successor": successor,
            "configuration_successor_verifier": {
                "path": verifier_relative.as_posix(), "bytes": len(verifier_raw),
                "sha256": lower_sha256(verifier_raw)},
