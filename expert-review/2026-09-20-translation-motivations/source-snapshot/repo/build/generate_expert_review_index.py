...
try:  # Both package tests and direct script execution are supported.
    from build import cardinality_repair_batch_20260908 as cardinality_batch
    from build import three_exact_locator_amendments_20260909 as exact_locators
    from build import reviewer_input_inventory_20260909 as amendment_inventory
    from build import msa_mp_nfc_transition_20260909 as mp_nfc
    from build import olp0014_note_review_20260912 as domain_note
except ModuleNotFoundError:
    import cardinality_repair_batch_20260908 as cardinality_batch
    import three_exact_locator_amendments_20260909 as exact_locators
    import reviewer_input_inventory_20260909 as amendment_inventory
    import msa_mp_nfc_transition_20260909 as mp_nfc
    import olp0014_note_review_20260912 as domain_note
...
EXPERT_AMENDMENT_SCHEMA_V2 = "openlogic-ar-expert-review-amendments-v2"
...
CARDINALITY_REPAIR_LEDGERS = {
    cardinality_batch.REPAIRS + c.stem + ".json": c for c in cardinality_batch.CONTRACTS
}
APPLIED_REPAIR_LEDGERS.update(CARDINALITY_REPAIR_LEDGERS)
FINITE_REVIEW_LEDGERS = set(NEXT_BATCH_LEDGERS) | set(CARDINALITY_REPAIR_LEDGERS)
CARDINALITY_CONFIG_SHA256 = "C20B63C3DC3AA97E56CE0E87DC6B3FBD303390449AB7EE8F93C7B922A35EFA6B"
# Selected exact English source lines, each contained in a preserved witness.
# The two pairing-alt quotations disclose two different source corrections.
CARDINALITY_ENGLISH_LINES = dict(zip(
    (d for c in cardinality_batch.CONTRACTS for d in c.decision_ids),
    (((41, 43),), ((41, 43),), ((92, 94),), ((92, 94),), ((100, 103),),
     ((19, 22), (39, 40)), ((19, 22), (39, 40)),
     ((20, 23),), ((33, 38),), ((87, 93),), ((33, 38),), ((62, 63),), ((140, 144),),
     ((99, 100),), ((99, 100),), ((29, 31),), ((79, 80),),
     ((47, 49),), ((47, 49),), ((51, 54),), ((72, 77),), ((87, 94),), ((13, 16),)),
    strict=True,
))
...
def validate_amendment_canon_evidence(
    root: Path, entry: dict, witness_excerpts: list[str], hashes: dict[Path, str],
) -> None:
    """Admit finite, byte-bound manual canon evidence, not machine attestation.

    A scanned page's transcription and semantic comparison are explicitly
    editorial evidence. Checks prove identities, declared scope and literal
    correspondence; they cannot prove that a person read or transcribed it.
    """
    identifier = entry["decision_id"]
    def check(condition: bool, message: str) -> None:
        if not condition:
            raise ValueError(f"{identifier}: canon evidence {message}")
    def nonempty(value: object) -> bool:
        return isinstance(value, str) and bool(value.strip())
    def bound_file(item: object, suffix: str) -> Path:
        check(isinstance(item, dict), "file identity is malformed")
        relative = item.get("path")
        check(isinstance(relative, str) and "\\" not in relative
              and not Path(relative).is_absolute(), "path must be a bounded relative path")
        target = (root / relative).resolve()
        allowed = ((root / "evidence").resolve(), (root.parent / "sources").resolve())
        if suffix == ".png":
            allowed += ((root / "tmp/pdfs").resolve(),)
        check(any(target.is_relative_to(base) for base in allowed)
              and target.suffix.lower() == suffix and target.is_file(), "file outside canon roots or missing")
        check(type(item.get("bytes")) is int and item["bytes"] > 0
              and target.stat().st_size == item["bytes"], "file byte count changed")
        expected_hash = str(item.get("sha256", "")).upper()
        check(bool(re.fullmatch(r"[0-9A-F]{64}", expected_hash)), "file hash malformed")
        actual = hashes.get(target) or sha256_file(target)
        check(actual == expected_hash, "file hash changed")
        hashes[target] = actual
        return target
    evidence = entry.get("canon_evidence")
    check(isinstance(evidence, list) and 0 < len(evidence) <= 20, "requires a bounded per-choice list")
    check(type(entry.get("official_attestation_claimed")) is bool, "claim must be boolean")
    observed = entry.get("observed_arabic_forms") or []
    chosen_forms = {item["form"] for item in observed} if observed else {entry["expected"]["chosen_arabic"]}
    exact_forms: set[str] = set()
    for item in evidence:
        check(isinstance(item, dict), "item malformed")
        for field in ("printed_page", "entry", "exact_passage", "chosen_arabic_form",
                      "attested_arabic_form", "english_headword", "english_source_phrase",
                      "english_sense", "relevance", "limits"):
            check(nonempty(item.get(field)), "missing " + field)
        check(type(item.get("physical_page")) is int and item["physical_page"] > 0,
              "physical page must be one-based")
        check(len(item["exact_passage"]) <= 1200, "passage is not short")
        check(sha256_bytes(item["exact_passage"].encode("utf-8")) ==
              str(item.get("passage_sha256", "")).upper(), "passage hash changed")
        check(item.get("transcription_mode") == "manual-visual-transcription-diacritics-normalized",
              "transcription method must be explicit")
        check(item.get("consulted_before_assessment") is True
              and item.get("recording_mode") == "retrospective-canon-check",
              "must disclose retrospective consultation before this assessment")
        try:
            consulted, assessed = date.fromisoformat(item["consulted_on"]), date.fromisoformat(entry["assessed_on"])
        except (KeyError, TypeError, ValueError) as exc:
            raise ValueError(f"{identifier}: canon evidence consultation date invalid") from exc
        check(consulted <= assessed, "consultation postdates assessment")
        source = item.get("source")
        bound_file(source, ".pdf")
        for field in ("title", "source_id", "url"):
            check(nonempty(source.get(field)), "source lacks " + field)
        check(bool(re.match(r"https?://[^\s<>]+$", source["url"])), "source URL malformed")
        bound_file(item.get("page_image"), ".png")
        check(item.get("relation") in {"exact-chosen-form", "related-not-exact"}, "relation invalid")
        check(item["english_sense"] == entry["changes"]["sense"], "sense differs from assessment")
        check(item["chosen_arabic_form"] in chosen_forms, "does not name a current chosen/displayed form")
        check(item["attested_arabic_form"] in item["exact_passage"]
              and item["english_headword"].casefold() in item["exact_passage"].casefold(),
              "dictionary forms absent from transcribed passage")
        for field, english in (("source_witness_indices", False), ("english_witness_indices", True)):
            indices = item.get(field)
            check(isinstance(indices, list) and bool(indices) and len(indices) == len(set(indices))
                  and all(type(i) is int and 0 <= i < len(entry["witnesses"]) for i in indices),
                  "context witness indices invalid")
            for index in indices:
                witness = entry["witnesses"][index]
                check((witness["source_kind"] == "english") == english, "context witness language differs")
                literal = item["english_source_phrase"] if english else item["chosen_arabic_form"]
                context = strip_tex_comments(witness_excerpts[index])
                pattern = r"(?<![\w-])" + re.escape(literal) + r"(?![\w-])"
                check(bool(re.search(pattern, context, re.IGNORECASE if english else 0)),
                      "literal current form absent from active context witness")
        if item["relation"] == "exact-chosen-form":
            check(item["attested_arabic_form"] == item["chosen_arabic_form"],
                  "exact relation cannot attest a different Arabic form")
            exact_forms.add(item["chosen_arabic_form"])
    if entry["official_attestation_claimed"]:
        check(chosen_forms <= exact_forms, "official claim lacks exact evidence for every current form")


def apply_expert_review_amendments(
    repo: Path, decisions: list[dict], english_root: Path | None,
    baseline_units: Sequence[dict] = (),
    *, frozen_inventory: dict | None = None,
    ...
    paths = (amendment_inventory.verify(root, frozen_inventory) if frozen_inventory is not None
             else sorted(directory.glob("*.json")))
    for path in paths:
        payload, raw = read_json_bytes(path)
        logical = path.relative_to(root).as_posix()
        if not isinstance(payload, dict) or payload.get("schema") not in {
            EXPERT_AMENDMENT_SCHEMA, EXPERT_AMENDMENT_SCHEMA_V2,
        }:
        ...
        for entry in entries:
            ...
            if {"amendment_schema", "original_fields", "ledger_path"}.intersection(entry):
                raise ValueError(f"{logical}: reserved generated assessment provenance")
            ...
            if (entry.get("recording_mode") != "new-independent-semantic-assessment"
                    ...
                    or (payload["schema"] == EXPERT_AMENDMENT_SCHEMA
                        and entry.get("official_attestation_claimed") is not False)
            ...
            fields = {"sense", "rationale", "expert_question"}
            promoted = {"expert_review_useful", "expert_review_reason"}
            allowed_changes = [fields]
            if payload["schema"] == EXPERT_AMENDMENT_SCHEMA_V2:
                allowed_changes.append(fields | promoted)
            if (not isinstance(changes, dict) or set(changes) not in allowed_changes
                    or any(not isinstance(changes.get(key), str) or not changes[key].strip()
                           for key in fields)
                    or (promoted.intersection(changes) and
                        (changes.get("expert_review_useful") is not True
                         or not isinstance(changes.get("expert_review_reason"), str)
                         or not changes["expert_review_reason"].strip()))):
            ...
            if payload["schema"] == EXPERT_AMENDMENT_SCHEMA_V2:
                validate_amendment_canon_evidence(root, entry, witness_excerpts, hashes)
                history = entry.get("historical_fields", {})
                # Retained snapshots have the rendered global-binding array;
                # fresh ledger decisions acquire it later in build_payload.
                # Derive only its proved empty pre-render value. An explicit
                # value or any pending global declaration must still compare
                # exactly, and the raw ledger decision is not rewritten.
                historical_values = dict(decision)
                if ("global_source_bindings" not in decision
                        and decision.get("_expert_global_source_raw", []) == []):
                    historical_values["global_source_bindings"] = []
                if not isinstance(history, dict) or any(historical_values.get(k) != v for k, v in history.items()):
                    raise ValueError(f"{decision_id}: historical fields changed")
            prepared = copy.deepcopy(entry)
            if payload["schema"] == EXPERT_AMENDMENT_SCHEMA_V2:
                prepared["amendment_schema"] = EXPERT_AMENDMENT_SCHEMA_V2
    ...
    for decision, entry, logical in plans:
        ...
        if entry.get("amendment_schema") == EXPERT_AMENDMENT_SCHEMA_V2:
            before["official_attestation_claimed"] = copy.deepcopy(decision.get("official_attestation_claimed"))
            for field in ("expert_review_useful", "expert_review_reason"):
                if field in entry["changes"]:
                    before[field] = copy.deepcopy(decision.get(field))
        decision.update(entry["changes"])
        decision["recording_mode"] = entry["recording_mode"]
        decision["open_to_correction"] = True
        if entry.get("amendment_schema") == EXPERT_AMENDMENT_SCHEMA_V2:
            decision["official_attestation_claimed"] = entry["official_attestation_claimed"]
...
class ExactReviewLiteral(str):
    """An already rendered formula passage; a second lossy render is forbidden."""
...
def human_chosen_arabic(decision: dict) -> str:
    ...
    if isinstance(display, dict) and collapse(display.get("chosen_arabic")):
        if (decision.get("semantic_propagation_repair") or {}).get("ledger_path") in FINITE_REVIEW_LEDGERS:
...
def canon_assessment_summary(assessment: dict) -> str:
    if assessment.get("amendment_schema") != EXPERT_AMENDMENT_SCHEMA_V2:
        return ""
    parts = []
    for item in assessment["canon_evidence"]:
        status = ("Exact chosen-form attestation" if item["relation"] == "exact-chosen-form"
                  else "Related canon evidence; current form is not literally attested")
        parts.append(
            f"{status}: {item['chosen_arabic_form']}. "
            f"Retrospective canon check, consulted {item['consulted_on']} before this assessment: "
            f"{item['source']['title']} (source ID {item['source']['source_id']}; {item['source']['url']}), "
            f"entry {item['entry']}, physical PDF page {item['physical_page']}, "
            f"printed page {item['printed_page']}. Passage (manual visual transcription, diacritics normalized): "
            f"{item['exact_passage']}. Relevance: {item['relevance']} Limits: {item['limits']}"
        )
    return render_literal_set_difference(" ".join(parts))


def human_decision_field(decision: dict, field: str) -> str:
    display = decision.get("review_display")
    if isinstance(display, dict) and (decision.get("semantic_propagation_repair") or {}).get("ledger_path") in FINITE_REVIEW_LEDGERS:
    ...
    if field == "rationale" and isinstance(assessment, dict):
        return (
            ...
            + value
            + (" " + canon_assessment_summary(assessment) if canon_assessment_summary(assessment) else "")
        )
    return value
...
def append_assessment_provenance(
    ...
    if isinstance(propagation, dict):
        ...
        for passage_index, passage in enumerate(decision.get("literal_source_passages", [])):
            ...
            if propagation.get("ledger_path") in FINITE_REVIEW_LEDGERS:
                shown = {key: ExactReviewLiteral(value) if isinstance(value, str) else value
            ...
            for extra_index, extra in enumerate(passage.get("additional_english", [])):
                label = shown["additional_english"][extra_index]
                lines.append("  - **Additional English evidence:** " + md_text(label)
                             + f" (EN L{extra['line_start']}–{extra['line_end']}; "
                             + "exact frozen-source witness retained in the index).")
        if propagation.get("cardinality_batch"):
            lines.append("- **Current source status:** Applied source correction, open to correction; "
                         "no rebuilt PDF or final reader page is claimed. Recorded decision status: "
                         + md_text(decision["status"]) + ".")
            if propagation.get("proposal_history"):
                prior = propagation["proposal_history"]
                href = arabic_prefix + quote(prior["history_source"]["path"], safe="/")
                lines.append("- **Proposal history:** This exact choice was previously proposed-unapplied "
                             "and is now applied in source (provisional-in-use). Its original motive is "
                             "unchanged; the historical proposal is not an outstanding unapplied choice. "
                             + f"[Preserved proposal ledger]({href}).")
        if propagation.get("cardinality_witness_transition"):
            lines.append("- **Historical assessment, current locations:** The original OLP-0033 rationale "
                         "and source declarations are retained unchanged. Exact inverse/forward evidence "
                         "connects their historical source to the later cardinality repairs; links identify "
                         "the unchanged qualification phrase in the current source, not the old full excerpt.")
        if propagation.get("encoding_transition"):
            transition = propagation["encoding_transition"]
            href = arabic_prefix + quote(transition["receipt_path"], safe="/")
            lines.append("- **Encoding provenance:** The original assessment and spelling remain in the record. "
                         "Current source links use canonically equivalent combining-mark order, verified by one exact "
                         f"reversible byte patch, not a new translation. [Exact transition]({href}).")
    assessment = decision.get("expert_review_assessment")
    ...
    if assessment.get("amendment_schema") == EXPERT_AMENDMENT_SCHEMA_V2:
        lines.append("- **Fresh canon evidence / شاهد اصطلاحي جديد:** "
                     + md_text(canon_assessment_summary(assessment)))
        for item in assessment["canon_evidence"]:
            lines.append("  - [Primary dictionary / المعجم الأصلي](" + item["source"]["url"] + ")"
                         + "; source ID: " + md_text(item["source"]["source_id"])
                         + "; physical PDF page " + str(item["physical_page"])
                         + ", printed page " + md_text(item["printed_page"]) + ".")
...
def render_literal_set_difference(text: str) -> str:
    """Display the exact A-backslash-B prose notation as ordered A∖B.

    The canon's difference assessment spells the mathematical operation with
    a literal backslash, not a TeX control sequence named B. Keep the same U+2216
    glyph as our existing setminus adapter, before its generic command fallback
    could erase this operator. Do not reinterpret paths, longer commands or TeX
    linebreaks. Raw ledger and canon evidence strings remain unchanged.
    """
    return re.sub(r"(?<![\w\\/:])A\\B(?![\w\\/])", "A∖B", text)
...
def humanize_tex(value: object, registry: TokenRegistry | None = None) -> str:
    ...
    text = render_literal_set_difference(collapse(value))
...
def render_next_source_review_text(value: object, registry: TokenRegistry) -> str:
    """Finite readable projection; the complete raw fields remain in the ledger.

    The meanings of pAssign/pValue, truth values, Log and PosInt are checked in
    source/open-logic-config.sty. Unsupported commands fail, never disappear.
    Typography/list markup is not a mathematical operator. Labels stay labels,
    not invented figure or page numbers.
    """
    text = expand_registered_tokens(value, registry)[0]
    if re.search(r"!!(?:\^|a|\{)", text):
        raise ValueError("Unregistered token in finite next-batch display")
    text = re.sub(r"\\pValue\s*(?:\{v\}|v)(?![A-Za-z])", "𝔳̅", text)
    text = re.sub(r"\\pAssign\s*(?:\{v\}|v)(?![A-Za-z])", "𝔳", text)
    text = re.sub(r"\\Log\s*\{L\}", "𝐋", text)
    ...
    for _ in range(4):
        updated = re.sub(r"\\(?:emph|textbf|textit|text|mathrm)\s*\{([^{}]*)\}", r"\1", text)
        if updated == text:
            break
        text = updated
    symbols = {
    ...
    def command(match):
        ...
        if name not in symbols:
            raise ValueError("Unsupported next-batch display command: " + name + " in " + repr(text))
        return symbols[name]
    # TeX control words stop before a digit: \ge2 is \ge followed by 2.
    # A regex word boundary would incorrectly leave that common form raw.
...
def render_cardinality_review_text(value: object, registry: TokenRegistry) -> str:
    """Finite cardinality notation, as defined by open-logic-config.sty.

    In particular comp{f}{g} means g composed with f, Bin is the binary set
    (blackboard B), and the English ordinal suffix is not exponentiation.
    Unsupported commands still fail through the existing strict renderer.
    """
    if not any(s["path"] == "source/open-logic-config.sty" and s["sha256"] == CARDINALITY_CONFIG_SHA256
               for s in registry.identity["sources"]):
        raise ValueError("Cardinality notation definitions differ from the checked registry")
    text = expand_registered_tokens(value, registry)[0]
    operators = {"tuple": (1, lambda a: "⟨" + a[0] + "⟩"),
        "Pow": (1, lambda a: "℘(" + a[0] + ")"),
        "comp": (2, lambda a: "(" + a[1] + " ∘ " + a[0] + ")"),
        "cardeq": (2, lambda a: "(" + a[0] + " ≈ " + a[1] + ")"),
        "frac": (2, lambda a: "((" + a[0] + ")/(" + a[1] + "))"),
        "overline": (1, lambda a: a[0] + "̅" if len(a[0]) == 1 else "overline(" + a[0] + ")")}
    for _ in range(64):
        match = re.search(r"\\(tuple|Pow|comp|cardeq|frac|overline)(?![A-Za-z])\s*", text)
        if not match:
            break
        arity, project = operators[match[1]]
        position, arguments = match.end(), []
        for _ in range(arity):
            while position < len(text) and text[position].isspace():
                position += 1
            group = parse_braced(text, position)
            if group is None:
                raise ValueError("Malformed finite cardinality operator: " + match[1])
            argument, position = group
            arguments.append(argument)
        text = text[:match.start()] + project(arguments) + text[position:]
    else:
        raise ValueError("Oversized finite cardinality operator expression")
    text = text.replace(r"^\text{th}", "ᵗʰ").replace("^{-1}", "⁻¹")
    symbols = {"Bin": "𝔹", "omega": "ω", "emptyset": "∅", "subseteq": "⊆",
               "setminus": "∖", "bigcup": "⋃", "infty": "∞", "times": "×", "in": "∈"}
    text = re.sub(r"\\(" + "|".join(symbols) + r")(?![A-Za-z])",
                  lambda match: symbols[match[1]], text)
    text = text.replace(r"\[", " ").replace(r"\]", " ")
    return render_next_source_review_text(text, registry)
...
def attach_review_display(
    ...
    if repair.get("ledger_path") in FINITE_REVIEW_LEDGERS:
        # Render this finite formula-bearing batch without either leaking TeX
        # or dropping operators through the legacy command-name fallback.
        def literal_display(value, language):
            renderer = (render_cardinality_review_text if repair.get("cardinality_batch")
                        else render_next_source_review_text)
            return renderer(value, registries[language])
        def alternative_display(value):
            if isinstance(value, str):
                return literal_display(value, "arabic")
            if isinstance(value, list):
                return [alternative_display(item) for item in value]
            if isinstance(value, dict):
                projected = {key: alternative_display(item) for key, item in value.items()}
                if repair.get("cardinality_batch") and "arabic" in projected:
                    projected.setdefault("form", projected["arabic"])
                return projected
        ...
        decision["review_display"].update(alternatives=alternative_display(decision.get("alternatives", [])),
            registry_expansions={}, raw_fields_preserved=True,
            literal_source_passages=[dict({phase: literal_display(p[phase]["literal"],
                "english" if phase == "english" else "arabic") for phase in ("english", "before", "after")},
                additional_english=[literal_display(w["literal"], "english") for w in p.get("additional_english", [])])
...
def discover_ledgers(repo: Path) -> list[Path]:
    ...
    # This finite nested record is the only newly commissioned note ledger.
    note_path = repo / domain_note.LEDGER_PATH
...
def normalize_semantic_propagation_repairs(
    ...
    for row in rows:
        ...
        for kind, field, prefix in (
            ...
            live_data = data
            if kind == "msa":
                data, encoding_transition, transition_hashes = mp_nfc.predecessor(
                    repo, unit_id, logical, declaration, live_data)
                hashes.update(transition_hashes)
            ...
            hashes[target] = sha256_bytes(live_data)
            live_excerpt = "\n".join(live_data.decode("utf-8-sig").splitlines()[start - 1:end])
            occurrence[kind] = structured_occurrence(
                unit_id, kind, expected_path, sha256_bytes(live_data), [(start, end)],
                "dated-propagation-repair-live-hash-and-term-verified", live_excerpt,
            )[kind]
            if kind == "msa":
        ...
        if encoding_transition is not None:
            normalized[-1]["semantic_propagation_repair"]["encoding_transition"] = encoding_transition
    identity["adapter"] = {
...
def normalize_qualification_propagation_repairs(
    ...
    for row in rows:
        ...
        cardinality_witness_transition = None
        for kind, field, prefix, term in (
            ...
            recorded = declaration.get("after_sha256" if kind == "msa" else "sha256")
            if unit_id == "OLP-0033" and kind in {"msa", "classical"} and digest != str(recorded).upper():
                admitted = cardinality_batch.normalize(repo, english_root)
                hashes.update({Path(item.path): item.sha256.upper() for item in admitted.input_identities})
                cardinality_source = next(s for s in admitted.sources if s.unit_id == unit_id and s.edition == kind)
                if (cardinality_source.after.path != expected_path
                        or cardinality_source.after.sha256.upper() != live_digest
                        or cardinality_source.before.sha256.upper() != str(recorded).upper()):
                    raise ValueError("OLP0033 qualification cardinality predecessor differs")
                data = cardinality_source.before_bytes
                digest = sha256_bytes(data)
                ledger = next(l for l in admitted.ledgers if l.identity.path == cardinality_source.ledger_path)
                if cardinality_witness_transition is None:
                    cardinality_witness_transition = {"ledger_path": ledger.identity.path,
                        "ledger_sha256": ledger.identity.sha256.upper(),
                        "historical_declarations": {}, "current_declarations": {}}
                cardinality_witness_transition["historical_declarations"][kind] = copy.deepcopy(declaration)
            ...
            if cardinality_source is not None:
                # Keep all checks above on exact historical assessed-after bytes.
                # Project only the unchanged selected qualification line, never
                # silently substitute a changed whole context or a later match.
                current_witnesses = []
                for first, last in term_ranges:
                    literal = b"".join(data.splitlines(keepends=True)[first-1:last]).rstrip(b"\r\n").decode("utf-8")
                    old_loc = cardinality_batch.exact_location(data, cardinality_source.before, literal)
                    if any(p.before.byte_start < old_loc.byte_end and old_loc.byte_start < p.before.byte_end
                           for p in cardinality_source.patches):
                        raise ValueError("Cardinality patch overlaps the historical qualification line")
                    new_loc = cardinality_batch.exact_location(cardinality_source.after_bytes, cardinality_source.after, literal)
                    expected_byte = old_loc.byte_start + sum(
                        len(p.after.literal.encode()) - len(p.before.literal.encode())
                        for p in cardinality_source.patches if p.before.byte_end <= old_loc.byte_start)
                    if new_loc.byte_start != expected_byte:
                        raise ValueError("Historical qualification line moved beyond the exact patch map")
                    current_witnesses.append(cardinality_location(new_loc, english_root))
                if len(current_witnesses) != 1:
                    raise ValueError("Unexpected cardinality qualification witness inventory")
                occurrence[kind] = cardinality_occurrence(unit_id, kind, current_witnesses)
                cardinality_witness_transition["current_declarations"][kind] = current_witnesses[0]
        normalized.append({
            "decision_id": f"semantic-propagation-20260906:{finding_id}",
            "recorded_decision_id": finding_id,
        ...
        if cardinality_witness_transition is not None:
            if set(cardinality_witness_transition["current_declarations"]) != {"msa", "classical"}:
                raise ValueError("Incomplete OLP0033 historical-to-current source transition")
            normalized[-1]["semantic_propagation_repair"]["cardinality_witness_transition"] = cardinality_witness_transition
...
def merge_exact_semantic_duplicates(decisions: Sequence[dict]) -> tuple[list[dict], int]:
    ...
    untouched: list[dict] = []
    for decision in decisions:
        if decision.get("semantic_propagation_repair") or decision.get("historical_proposal"):
...
def normalize_applied_repairs(payload: dict, identity: dict, repo: Path,
    ...
    if identity["path"] not in APPLIED_REPAIR_LEDGERS and (
        ...
        or payload.get("schema") in {contract.schema for contract in CARDINALITY_REPAIR_LEDGERS.values()}
    ...
    if identity["path"] in CARDINALITY_REPAIR_LEDGERS:
        return normalize_cardinality_repairs(payload, identity, repo, baseline, english_root)
...
def cardinality_location(location, english_root):
    """Public source path plus exact byte and occupied-line selection."""
    path = Path(location.source.path)
    logical = path.relative_to(english_root.resolve()).as_posix() if path.is_absolute() else path.as_posix()
    return {"path": logical, "sha256": location.source.sha256.upper(), "bytes": location.source.bytes,
        "byte_start": location.byte_start, "byte_end": location.byte_end,
        "line_start": location.line_start, "line_end": location.line_end,
        "literal": location.literal, "excerpt": location.excerpt,
        "excerpt_sha256": sha256_bytes(location.excerpt.encode("utf-8"))}


def cardinality_occurrence(uid, kind, witnesses):
    first = witnesses[0]
    dated_repair_require(all(w["path"] == first["path"] and w["sha256"] == first["sha256"]
                             for w in witnesses), "mixed cardinality occurrence sources")
    group = structured_occurrence(uid, kind, first["path"], first["sha256"],
        [(w["line_start"], w["line_end"]) for w in witnesses],
        "dated-applied-cardinality-repair-exact-byte-phrase-verified")[kind]
    for locator, witness in zip(group["locators"], witnesses):
        locator["excerpt"] = witness["excerpt"]
    return group


def normalize_cardinality_repairs(payload, identity, repo, baseline, english_root):
    """23 current choices; seven original raw-ID proposals survive as history.

    The shared finite adapter proves bytes/ownership. This projection retains
    raw rationale, status and authority distinctions; it never infers approval.
    """
    dated_repair_require(english_root is not None, "cardinality English root required")
    batch = cardinality_batch.normalize(repo, english_root)
    hashes = {Path(i.path): i.sha256.upper() for i in batch.input_identities}
    config_path = (repo / "source/open-logic-config.sty").resolve()
    config = config_path.read_bytes()
    dated_repair_require(sha256_bytes(config) == CARDINALITY_CONFIG_SHA256,
                         "cardinality notation configuration differs")
    hashes[config_path] = CARDINALITY_CONFIG_SHA256
    ledger = next(l for l in batch.ledgers if l.identity.path == identity["path"])
    dated_repair_require(identity["sha256"].upper() == ledger.identity.sha256.upper()
        and identity["bytes"] == ledger.identity.bytes and json.loads(ledger.raw_bytes) == payload,
        "cardinality supplied payload differs from pinned ledger")
    transitions = {(s.unit_id, s.edition): s for s in batch.sources}
    supersessions = {s.decision_id: s for s in batch.supersessions}
    result, historical = [], []
    for choice in ledger.decisions:
        raw = json.loads(choice.raw_json)
        uid, kind = choice.unit_id, choice.edition
        source = transitions[(uid, kind)]
        expected = baseline[uid]
        dated_repair_require(source.after.path == expected["arabic_path" if kind == "msa" else "target_path"],
                             "cardinality Arabic baseline path differs")
        english_path = (english_root / expected["source_path"]).resolve()
        english_data = english_path.read_bytes()
        dated_repair_require(sha256_bytes(english_data) == str(expected["english_sha256"]).upper()
            == hashes[english_path], "cardinality frozen English baseline differs")
        english_identity = cardinality_batch.Identity(english_path.as_posix(), len(english_data),
                                                     cardinality_batch.sha(english_data))
        english = []
        source_lines = english_data.splitlines(keepends=True)
        for start, end in CARDINALITY_ENGLISH_LINES[choice.decision_id]:
            first_byte = len(b"".join(source_lines[:start-1]))
            last_byte = len(b"".join(source_lines[:end]).rstrip(b"\r\n"))
            candidates = {(max(first_byte, w.location.byte_start), min(last_byte, w.location.byte_end))
                for w in choice.witnesses if w.location.source.sha256 == english_identity.sha256
                and w.location.line_start <= start <= end <= w.location.line_end}
            dated_repair_require(len(candidates) == 1, "ambiguous cardinality English context selection")
            first_byte, last_byte = candidates.pop()
            selected = english_data[first_byte:last_byte].rstrip(b"\r\n")
            location = cardinality_batch.exact_location(english_data, english_identity, selected.decode())
            dated_repair_require((location.line_start, location.line_end) == (start, end)
                and any(w.location.source.sha256 == english_identity.sha256
                    and w.location.byte_start <= location.byte_start < location.byte_end <= w.location.byte_end
                    for w in choice.witnesses), "selected English lines outside exact cardinality witness: " + choice.decision_id)
            english.append(cardinality_location(location, english_root))
        passages, occurrences = [], []
        for patch in choice.patches:
            before, after = (cardinality_location(loc, english_root) for loc in (patch.before, patch.after))
            passage = {"edition": kind, "patch_index": patch.application_index, "patch_id": patch.patch_id,
                "english": english[0], "before": before, "after": after}
            if len(english) > 1:
                passage["additional_english"] = english[1:]
            passages.append(passage)
            occurrences.append({"unit_id": uid, "context_note": "Exact applied cardinality correction: " + patch.patch_id,
                "preserve_literal_pair": True, "english": cardinality_occurrence(uid, "english", english),
                kind: cardinality_occurrence(uid, kind, [after])})
        history = {"path": source.before.path, "sha256": source.before.sha256.upper(), "bytes": source.before.bytes,
            "text_utf8": source.before_bytes.decode("utf-8"),
            "patches": [json.loads(p.raw_json) for p in source.patches],
            "status": "historical-before-bytes-proved-by-exact-inverse-not-current-source"}
        record = copy.deepcopy(raw)
        record.update(decision_id="semantic-propagation-20260907:" + choice.decision_id,
            recorded_decision_id=choice.decision_id, before_arabic="\n\n".join(p.before.literal for p in choice.patches),
            expert_question=raw.get("expert_question") or raw.get("expert_question_ar"),
            expert_review_non_blocking=True,
            assessed_on=payload["assessed_on"], recording_mode=choice.recording_mode,
            source_record=identity, occurrences=occurrences, literal_source_passages=passages,
            printed_page=None, page_status="bind-after-final-reader-build",
            semantic_propagation_repair={"ledger_path": identity["path"], "ledger_sha256": identity["sha256"],
                "assessed_on": payload["assessed_on"], "finding_id": choice.decision_id, "changed_edition": kind,
                "source_ledger": copy.deepcopy(payload), "choice_record": copy.deepcopy(raw),
                "historical_before_source": {"sources": [history], "status": history["status"]},
                "cardinality_batch": "exact-cardinality-repair-batch-20260908",
                "source_order_patch_ids": list(source.source_order_patch_ids),
                "verified_witnesses": [{"location": cardinality_location(w.location, english_root),
                    "recorded_phase": w.recorded_phase, "verified_phase": w.verified_phase,
                    "raw_record": json.loads(w.raw_json)} for w in choice.witnesses]})
        record.setdefault("official_attestation_claimed", False)
        record.setdefault("basis", "contextual-semantic-source-correction")
        record.setdefault("alternatives", [])
        if choice.decision_id in supersessions:
            old = supersessions[choice.decision_id]
            history_identity = {"path": old.history_source.path, "sha256": old.history_source.sha256.upper(),
                                "bytes": old.history_source.bytes}
            prior = {"history_source": history_identity, "previous_status": old.previous_status,
                "current_status": old.current_status, "changed_fields": list(old.changed_fields),
                "original_choice": json.loads(old.previous_decision_json)}
            record["semantic_propagation_repair"]["proposal_history"] = copy.deepcopy(prior)
            historical_row = copy.deepcopy(prior["original_choice"])
            historical_row.update(source_record=history_identity, historical_proposal=prior,
                human_superseded_by_authoritative_repair=record["decision_id"],
                historical_status_note="Preserved unapplied proposal at its historical date; this exact choice is now applied in the linked current record.")
            historical.append(historical_row)
        result.append(record)
    dated_repair_require(len(result) == len(ledger.decisions), "cardinality current decision count differs")
    identity["adapter"] = {"kind": "dated-cardinality-source-batch-20260908",
        "terminology_rows_contributed": len(result), "historical_proposal_rows_preserved": len(historical),
        "current_patch_occurrences": sum(len(r["occurrences"]) for r in result),
        "historical_motives_or_new_attestation_claimed": False}
    return result + historical, hashes
...
def load_decisions(
    ...
    for path in paths:
        ...
        if relative == domain_note.LEDGER_PATH:
            rows, note_hashes = domain_note.normalize(repo, english_root, payload, identity)
            applied_hashes.update(note_hashes)
        ...
        for original in rows:
            ...
            if (decision.get("source_clarification") or str(decision_id).startswith(domain_note.PREFIX)) and (
                relative != domain_note.LEDGER_PATH or decision_variants[decision_id]
            ):
                raise ValueError("uncommissioned/duplicate source clarification: " + str(decision_id))
            ...
            if decision.get("historical_proposal"):
                if not is_propagation_row or relative not in CARDINALITY_REPAIR_LEDGERS:
                    raise ValueError("Unvalidated historical cardinality proposal")
                decision["source_record"] = copy.deepcopy(decision["historical_proposal"]["history_source"])
                if decision["source_record"] not in inventory:
                    inventory.append(decision["source_record"])
            else:
...
def alternatives_text(decision: dict) -> list[str]:
    assessment = decision.get("expert_review_assessment") or {}
    editorial = assessment.get("current_editorial_assessment") or {}
    comparison = collapse(editorial.get("alternative_comparison"))
    current = ([f"Current alternative comparison ({assessment['assessed_on']}; open to correction): "
    ...
    # Keep the former ledger verbatim, but do not present its old 'no alternative'
    # claim as the current conclusion after a new comparison has been recorded.
    if current:
        return current + ["Historical alternative record: " + item for item in rendered]
    return rendered
...
def review_question(decision: dict) -> str:
    if ((decision.get("semantic_propagation_repair") or {}).get("cardinality_batch")
            and decision.get("expert_review_useful") is False
            and not (decision.get("expert_question") or decision.get("expert_question_ar"))):
        return ("Mechanical NFC mark-order repair; the ledger records no separate expert question. "
                "Recorded reason: " + human_decision_field(decision, "expert_review_reason"))
    display = decision.get("review_display")
    explicit = (
...
def location_lines_markdown(
    ...
    for (source_kind, logical_path, file_url), rows in file_groups.items():
        ...
        for (status, hash_matches, _), bucket in resolved_buckets.items():
            ...
            elif status == "verified-explicit-locator-amendment":
                notes.append("exact lexical source witness; previous locator retained in the full record")
...
def occurrence_review_question(decision: dict, occurrence: dict | None) -> str:
    ...
    if cited_units and unit_id not in cited_units:
        base = default_review_question(decision)
        ...
        # Other cited units may be deliberate cross-section comparisons, not
        # evidence that the question belongs to a different occurrence.
...
def build_payload(
    ...
    assessment_identities, assessment_hashes = apply_expert_review_amendments(
        repo, decisions, english_root, units, frozen_inventory=frozen_inventory
    )
    source_hashes.update(assessment_hashes)
    exact_locator_batch = exact_locators.Batch(repo, english_root, decisions)
    source_hashes.update(exact_locator_batch.hashes)
    ...
    for decision in decisions:
        ...
        for occurrence_index, occurrence in enumerate(record.get("occurrences") or [], 1):
            ...
            exact_locator_batch.apply_frame(record["decision_id"], occurrence_index, occurrence, locations)
            for location in locations:
    ...
    payload = {
        ...
        "exact_locator_amendments": exact_locator_batch.finish(),
        "frozen_amendment_inventory": frozen_inventory,
...
def write_outputs(
    ...
    # Keep only references to the already-rendered text. Materializing the full
    # JSON string, its UTF-8 copy, and every surface's bytes at once exceeded the
    # bounded worker's memory allowance on the real consolidated index.
    text_outputs = {
        output_md: markdown,
        output_start_here_md: start_here_markdown,
        output_priority_md: priority_markdown,
        output_csv: occurrence_csv,
    }
    for path in [output_json, *text_outputs]:
    ...
    all_text = {**text_outputs, **shard_text}
    ...
    try:
        ...
        for path in sorted([output_json, *all_text], key=lambda item: str(item)):
            temporary = path.with_name(
                "." + path.name + ".expert-review-index.tmp"
            )
            staged.append((temporary, path))
            encoding = "utf-8-sig" if path == output_csv else "utf-8"
            with temporary.open("w", encoding=encoding, newline="\n") as stream:
                if path == output_json:
                    # json.dump iterates encoder chunks instead of joining the
                    # complete document. Settings and final newline are byte-
                    # identical to the former dumps(...).encode('utf-8') path.
                    json.dump(serializable, stream, ensure_ascii=False,
                              indent=2, sort_keys=True)
                    stream.write("\n")
                else:
                    stream.write(all_text[path])
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--amendment-inventory", type=Path,
                        help="One frozen amendment inventory shared with independent readback")
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    frozen_inventory = (json.loads(args.amendment_inventory.read_text(encoding="utf-8-sig"))
                        if args.amendment_inventory else amendment_inventory.capture(repo))
    amendment_inventory.verify(repo, frozen_inventory)
    output_md = (
        args.output_md
