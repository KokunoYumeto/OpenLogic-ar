...
\dots{} تعدّد~$A$، فإن القائمة $f(x_1)$، $f(x_2)$، \dots{}، بعد حذف
التكرار وإعادة ترقيم ما بقي، تعدّد~$B$.
وفي حالتنا نبحث عن !!a{surjection} $f\colon \Pow{\Nat}
\to \Bin^\omega$.

\textbf{تنبيه تصحيحي على الأصل (C040-01).} قد تتكرر قيم الدالة الشاملة؛ فلا
تكون قائمة الصور تعدادًا تقابليًا على التعريف البديل إلا بعد حذف المكرر
وإعادة فهرسة ما بقي.
...
$f(N)$ السلسلة $s$ التي تحقق $s(n) = 1$ إذا وفقط إذا كان $n \in N$،
وتحقق $s(n) = 0$ فيما عدا ذلك.
...
\begin{editorial}
\textbf{تصويب في المصدر (OLSIZ-010).}
سمّى النص الإنجليزي المتتالية $s_k$ من غير أن يربط $k$؛ فاستُعمل الاسم
