...
\begin{proof}
  ...
  $\Domain{N} = \{b_0, b_1, \ldots \}$. نختار أولًا $p_0 \in I$،
  ثم نبني تشاكلات جزئية متزايدة
  $p_0 \subseteq p_1 \subseteq p_2 \subseteq \cdots$، فنناوب
...
\begin{prob}
  استوفِ تفاصيل البرهان على أن $p$ المعرفة في
...
\begin{proof}
  ...
  اختر عائلةً $I$ من التشاكلات الجزئية تشهد خاصية الذهاب والإياب.
  ونثبت بالاستقراء على الـ!!{formula}s الدعوى الآتية: إذا كانت
  متغيرات~$!A$ الحرة محصورة في $x_1$، \dots،~$x_n$، وكان $p\in I$
  يرسل كل $a_i$ إلى $b_i$، وكان $s_1(x_i)=a_i$ و$s_2(x_i)=b_i$
  لكل $i=1$، \dots،~$n$، فإن $\Sat{M}{!A}[s_1]$ إذا وفقط إذا كان
  $\Sat{N}{!A}[s_2]$. الحالة الذرية من كون~$p$ تشاكلًا جزئيًا،
  وحالات الروابط مباشرة. وفي حالتي المكمّمين نستعمل خاصيتي الذهاب
  والإياب لمدّ~$p$ إلى $q\in I$ يضم الشاهد الجديد في مجاله أو مداه،
  ثم نطبق فرض الاستقراء. فإذا كان~$!A$ جملة أخذنا $n=0$ وأي~$p\in I$،
  فحصل $\Struct{M} \elemequiv \Struct{N}$.
\end{proof}

\begin{rem}
ولا تبطل النتيجة إذا كانت في اللغة !!{function}s؛ وإنما ننظر
حينئذ في التشاكل الذي تولّده $p$ بين الـ!!{structure} الجزئية
من $\Struct{M}$ المولّدة بالعناصر $a_1$، \dots، $a_n$،
...
\begin{proof}
  نستقرئ على $n$، ونثبت الدعوى في كل مرحلة لجميع قيم $k$ معًا.
\end{proof}
...
\begin{proof}
  ...
  $I_n(\mathbf{a},\mathbf{b})$ تستلزم اتفاق $\mathbf{a}$ و$\mathbf{b}$
  في الـ!!{formula}s التي لا تزيد رتبة كمها على~$n$، والتي تقع
  متغيراتها الحرة في القائمة الثابتة الموافقة لإحداثياتهما.
  وأما العكس، فنستقرئ فيه على $n$، ونعتمد
  \olref{prop:qr-finite}. فهي تضمن، لكل $n$ ولكل قائمة منتهية
  ثابتة من المتغيرات الحرة، أن فئات التكافؤ المنطقي
  للـ!!{formula}s ذات رتبة الكم التي لا تتجاوز~$n$ منتهية العدد.

  عند $n=0$، نعلم بالفرض أن $\mathbf{a}$ و$\mathbf{b}$ تتفقان
  في الـ!!{formula}s الخالية من الكم؛ فتتفقان خاصة في الصيغ
  الذرية، وهذا بعينه $I_0(\mathbf{a},\mathbf{b})$.

  وللحالة $n+1$ نفرض اتفاق $\mathbf{a}$ و$\mathbf{b}$ في
  الـ!!{formula}s التي لا تزيد رتبة كمها على $n+1$.
  لإثبات $I_{n+1}(\mathbf{a},\mathbf{b})$ ننظر أولًا في جهة
  الذهاب: لكل $a \in \Domain M$ نطلب $b \in \Domain N$ بحيث
  $I_n(\mathbf{a}a,\mathbf{b}b)$. وبفرض الاستقراء يكفي أن
  نجد، لكل $a \in \Domain M$، عنصرًا $b \in \Domain N$
  تتفق عنده $\mathbf{a}a$ و$\mathbf{b}b$ في الـ!!{formula}s
  التي لا تزيد رتبة كمها على~$n$.

  فلنأخذ $a \in \Domain M$. نختار ممثلًا لكل فئة تكافؤ منطقي
  للـ!!{formula}s~$!B(x,\mathbf{y})$ التي لا تزيد رتبة كمها على~$n$،
  وتقع متغيراتها الحرة في القائمة الثابتة $x,\mathbf{y}$،
  وتحققها $\mathbf{a}a$ في $\Struct{M}$. ونجعل $!T^a_n$
  اقتران الممثلين المختارين. وهم عدد منتهٍ، فيكون الاقتران
  !!{formula} واحدة من الرتبة الأولى. وعندئذ تحقق $\mathbf{a}$
  الصيغة $\lexists[x][!T^a_n(x,\mathbf{y})]$، ولا تزيد رتبة
  كمها على $n+1$. فيلزم بالفرض أن تحقق $\mathbf{b}$ الصيغة
  نفسها في $\Struct{N}$؛ فيوجد $b \in \Domain N$ تحقق عنده
  $\mathbf{b}b$ الصيغة $!T^a_n$. ومن ثم تتفق $\mathbf{b}b$
  في الـ!!{formula}s ذات رتبة الكم التي لا تتجاوز~$n$
  مع $\mathbf{a}a$؛ إذ في المختار ممثل لكل صيغة صادقة،
  أو لنفيها إن كانت كاذبة. وبنظير هذا النظر نثبت جهة الإياب:
  لكل $b \in \Domain N$ يوجد $a\in \Domain M$ تتفق عنده
  $\mathbf{a}a$ و$\mathbf{b}b$ في الـ!!{formula}s ذات رتبة
  الكم التي لا تتجاوز~$n$. فتم البرهان.
\end{proof}
