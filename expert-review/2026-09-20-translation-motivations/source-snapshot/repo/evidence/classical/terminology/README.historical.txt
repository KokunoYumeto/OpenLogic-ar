...
## Start here: public expert review / ابدأ من هنا للمراجعة المتخصصة

Reviewers can use these surfaces in order:

1. [`EXPERT_REVIEW_START_HERE.md`](EXPERT_REVIEW_START_HERE.md) — a short, curated bilingual queue with three explicit tiers: **check first**, **specialist review**, then the **exhaustive archive**. Routine auto-mined prose cannot occupy the first screen. Each row shows the term, Arabic choice, one-line reason, a representative unit and exact source-line link where proved, plus a link to every remaining occurrence.
2. [`reviewer-index/README.md`](reviewer-index/README.md) — byte-bounded, browser-renderable priority and complete A–Z/ID shards. Each choice gives its sense, Arabic, short rationale, useful alternatives, a conspicuous “PLEASE DOUBLE-CHECK” prompt and every distinct recorded occurrence. Repeated unit headings are grouped, but their source locations are never discarded.
3. [`EXPERT_REVIEW_OCCURRENCES.csv`](EXPERT_REVIEW_OCCURRENCES.csv) — a sortable UTF-8-with-BOM table with one row per distinct recorded occurrence. Indistinguishable repeated ledger rows stay auditable in JSON but are not duplicated for reviewers. A choice whose ledger has no occurrence receives one explicit **location not recorded** row, so it cannot disappear from the sortable layer. The table combines EN/MSA/Classical source locators and clearly grades exact, fallback and unavailable evidence.
4. [`EXPERT_REVIEW_PRIORITY_ONLY.md`](EXPERT_REVIEW_PRIORITY_ONLY.md) and [`EXPERT_REVIEW_INDEX.md`](EXPERT_REVIEW_INDEX.md) — canonical, intentionally large monolithic Markdown sheets retained for preservation and bulk search.
5. [`EXPERT_REVIEW_INDEX.json`](EXPERT_REVIEW_INDEX.json) — the complete lossless machine record, including non-priority decisions, deduplicated-display provenance, excluded wrong-sense candidates and generic bookkeeping retained only in JSON.

Source-line links are available in source-only snapshots. Component printed-page and assembled-PDF-page fields are populated only after the final reader build supplies its PDF, AUX and SyncTeX evidence. Empty page fields are intentional and are never filled by estimation.
