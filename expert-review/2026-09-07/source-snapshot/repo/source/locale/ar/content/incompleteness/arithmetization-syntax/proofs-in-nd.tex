...
\begin{proof}
  ...
  صحيح. ولتكن $\fn{TreeCode}(d)$ الخاصية العودية البدائية التي تصدق
  إذا وفقط إذا كان~$d$ رمز شجرة منتهية ذات جذر بحسب ترميز الأشجار
  المعتمد. ومن ثم تصدق $\fn{Deriv}(d)$ إذا وفقط إذا
  \[
  \fn{TreeCode}(d) \land
...
\begin{proof}
  ...
  $\delta_{i+1}$ !!{derivation} جزئيًا مباشرًا من~$\delta_i$. فإذا كان
  $n=0$ كان الورود غير !!{discharged} بالتعريف؛ وإذا كان $n>0$ بقي غير مفرَّغ
  متى لم تنتهِ أي عقدة تسبق الفرض على هذا المسار باستدلال وسم تفريغه~$n$.

  كون !!{derivation} ما جزئيًا مباشرًا علاقة عودية بدائية: تصدق
  $\fn{Subderiv}(d,d')$ إذا وفقط إذا
  $\bexists{j<(d')_0}{d=(d')_{j+1}}$. ولتصدق $\fn{Path}(s,d)$ إذا كان
  $s$ رمز متتالية غير خالية، أولها~$d$، وكل عنصر تالٍ فيها !!{derivation}
  جزئيًا مباشرًا من سابقه؛ أي إذا وفقط إذا
  \[
    \fn{SeqCode}(s) \land \len{s}>0 \land (s)_0=d \land
    \bforall{i<(\len{s}\tsub 1)}{\fn{Subderiv}((s)_{i+1},(s)_i)}.
  \]
  وهذه علاقة عودية بدائية. ثم إن رمز كل شجرة جزئية مباشرة أصغر من رمز
  الشجرة التي تحتويها في ترميز المتتاليات المستعمل هنا؛ ومن ثم لا يزيد
  طول مسار يبدأ عند~$d$ على $d+1$، ولا يزيد أي عنصر فيه على~$d$.
  لذلك يحدّ $\fn{sequenceBound}(d,d+1)$ رمز كل مسار كهذا. ويمكننا تعريف
