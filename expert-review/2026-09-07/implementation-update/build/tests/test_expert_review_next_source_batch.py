...
class NextSourceBatch(unittest.TestCase):
    ...
    def test_formula_display_preserves_commands_and_bad_operator_is_rejected(self):
        key = next(k for k in self.rows if "FINITE-GRID" in k)
        row = self.rows[key]
        self.assertIn(r"m\ge2", G.human_chosen_arabic(row))
        self.assertIn(r"m\ge2", G.rtl(G.human_chosen_arabic(row)))
        wrong = copy.deepcopy(row)
        wrong["review_display"]["chosen_arabic"] = wrong["review_display"]["chosen_arabic"].replace(r"\ge", r"\le")
        self.assertTrue(H.record_errors(wrong, self.specs[key]))
